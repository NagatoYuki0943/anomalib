...
class PatchcoreModel(DynamicBufferModule, nn.Module):
    ...
    def reshape_embedding(embedding: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            embedding (torch.Tensor): Embedding tensor extracted from CNN features. [1, 384, 28, 28]
...
def my_cdist_p2(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
...
def my_cdist_p2_v1(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
    """这个函数主要是为了解决torch.cdist导出onnx后,使用其他推理引擎推理onnx内存占用过大的问题
        如果使用torchscript推理则不会有内存占用过大的问题,使用原本的torch.cdist即可
...
def my_cdist_p2_v2(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
    """这个函数主要是为了解决torch.cdist导出onnx后,使用其他推理引擎推理onnx内存占用过大的问题
        如果使用torchscript推理则不会有内存占用过大的问题,使用原本的torch.cdist即可
...
def fast_cdist(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:

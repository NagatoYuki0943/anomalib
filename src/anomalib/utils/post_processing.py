...
def anomaly_map_to_color_map(anomaly_map: np.ndarray, normalize: bool = True) -> np.ndarray:
    ...
    anomaly_map = cv2.applyColorMap(anomaly_map, cv2.COLORMAP_JET)  # [h, w] -> [h, w, 3]
...
def compute_mask(anomaly_map: np.ndarray, threshold: float, kernel_size: int = 1) -> np.ndarray:
